...
// recieving cart contents from HTML
app.get('/add-to-cart/:animalId', (req, res) => {
  
  const sess = req.session
  // setting animalId from JSON
  const animalId = req.params.animalId
  // checking for empty cart, if condition is met, create new empty object
  if (!sess.cart) {
    sess.cart = {}
  }
...
app.get('/cart', (req, res) => {
  // checking for empty cart, if condition is met, then create empty object
    if(!req.session.cart){
      req.session.cart = {};
    }
    // recieving cart contents from /cart
    const cart = req.session.cart
    // creating empty animal array
    const animals = []
    //setting order total to 0
    let orderTotal = 0;
    // looping through cart object
    for (const animalId in cart){
      // setting animalDetails to index of 
